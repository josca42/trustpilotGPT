...
from sdc.db import crudutils


def scrape_companies(max_pages_to_scrape=40):
    i = 0
...
def anonymize_and_dump_reviews2db(reviews, company):
    for review in reviews:
        ...
        if review_db is not None and (
            review_db.timestamp == review_obj.timestamp.replace(tzinfo=None)
        ):
            continue
        else:
            review_obj.embedding = cohere_embed(review_obj.content)[0]
            crud.company_review.upsert(review_obj)


if __name__ == "__main__":
    scrape_company_reviews()
    scrape_real_estate_agents()
